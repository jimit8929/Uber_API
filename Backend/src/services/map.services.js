...
export const calculateDistanceTime = async (originName, destinationName) => {
  try {
...
export const getAutoCompleteSuggestionservice = async (req, res) => {
    try {
        ...
        const response = await axios.get(url, {
            headers: { "User-Agent": "Jimit" } // Required by Nominatim
        });
...
export const getCaptainInTheRadius = async(lt , lng , radius) => {
